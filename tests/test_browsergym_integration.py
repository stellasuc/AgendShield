...
def test_shielded_agent_preserves_upstream_goal_and_chat_before_plan_step_check(tmp_path):
    ...
    action, info = shielded.get_action(observation)
...
def test_shielded_agent_sends_feedback_to_existing_agent_and_replans(tmp_path):
    ...
    assert records[0]["record_type"] == "PLAN_STEP_DECISION"
