...
`ShieldedBrowserAgent` 是组合适配器：它转发原 AWM 的 observation preprocessor 与 action set，不修改 AWM 的 Goal、Chat 或工作流 Prompt。AWM 生成的每个下一步动作被视为候选计划步骤，经过 AST、预期事件和规则电路核验后，才可能从适配器返回给 BrowserGym。

候选计划步骤核验位于 `get_action()` 返回与 `env.step()` 调用之间：它既不增加 AWM 的输入 token，也不改变上游 Agent 行为，同时仍在网页副作用发生前执行不可绕过的确定性门控。
